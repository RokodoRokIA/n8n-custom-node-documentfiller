...
exports.runPdfReActAgent = runPdfReActAgent;
const pdf_filler_service_1 = require("./pdf-filler.service");
 ...
 */
async function runPdfReActAgent(model, context, pdfBuffer) {
    ...
    while (iteration < MAX_ITERATIONS) {
        ...
        const prompt = buildPdfPrompt(missingFields, fieldConfig, availableLines, docType, sectionBounds);
        try {
            ...
            issues.push(`Erreur LLM: ${error.message}`);
            if (debug) {
        ...
        if (stillMissing.length > 0) {
            ...
            const heuristicPositions = detectPositionsHeuristic(pdfContent, stillMissing, fieldConfig, sectionBounds, targetPageNumber);
        ...
        const fillResult = await (0, pdf_filler_service_1.fillPdf)(currentPdfBuffer, matches, fillData, {
            fontSize: 9,
            debug: false,
        });
        ...
        const filledContent = await (0, pdf_extractor_service_1.extractPdfContent)(currentPdfBuffer, {
        ...
        const correctionPrompt = buildCorrectionPrompt(fieldsToCorrect, fieldConfig, availableLines, positions);
        try {
            ...
            issues.push(`Erreur correction LLM: ${error.message}`);
            if (debug) {
                console.log(`   ❌ Erreur correction LLM: ${error.message}`);
            }
    ...
    if (checkboxData && Object.keys(checkboxData).length > 0) {
        ...
        const checkboxResult = await (0, pdf_filler_service_1.fillCheckboxes)(currentPdfBuffer, pdfContent, checkboxData, fieldConfig, targetPageNumber, debug);
 ...
 */
function findSectionBounds(content, customMarker, docType) {
    const markers = [
        customMarker,
 ...
 */
function extractAvailableLines(content, sectionBounds, targetPage) {
    const lines = [];
    ...
    for (const page of pagesToAnalyze) {
        ...
        for (const line of sortedLines) {
            ...
            if (sectionBounds && page.pageNumber === sectionBounds.startPage) {
                if (line.y >= sectionBounds.startY)
                    continue;
            }
            lines.push({
 ...
 */
function buildPdfPrompt(missingFields, fieldConfig, availableLines, docType, sectionBounds) {
 ...
 */
function buildCorrectionPrompt(fieldsToCorrect, fieldConfig, availableLines, previousPositions) {
    var _a;
...
- Type attendu: ${(config === null || config === void 0 ? void 0 : config.type) || 'text'}
- Labels: ${((_a = config === null || config === void 0 ? void 0 : config.labels) === null || _a === void 0 ? void 0 : _a.join(', ')) || field.tag}
 ...
 */
function parseLLMPositions(response, _fieldConfig) {
    const positions = {};
    if (!response)
        return positions;
    // Extraire le JSON
    ...
    try {
        ...
        for (const item of positionsList) {
            if (!item.tag || typeof item.tag !== 'string')
                continue;
            if (typeof item.page !== 'number' || typeof item.y !== 'number')
                continue;
            positions[item.tag] = {
                page: item.page,
                x: item.x || 71,
        ...
        console.error('Erreur parsing JSON LLM:', error.message);
    }
    return positions;
}
/**
 ...
 */
function detectPositionsHeuristic(content, missingFields, fieldConfig, sectionBounds, targetPage) {
    var _a;
    const positions = {};
    const pagesToAnalyze = content.pages.filter(p => {
        if (sectionBounds) {
    ...
    for (const field of missingFields) {
        ...
        for (const page of pagesToAnalyze) {
            ...
            if (config.type === 'inline') {
                ...
                const lineElements = page.textElements.filter(el => Math.abs(el.y - firstOcc.line.y) < 5).sort((a, b) => a.x - b.x);
                ...
                for (const occ of occurrences) {
                    ...
                    if (endIndex + 1 < linesInSection.length) {
                        ...
                        const minGap = (_a = config.minGap) !== null && _a !== void 0 ? _a : 0;
                        if (gap >= minGap) {
                            const inputY = nextLine.y + (gap / 2);
                            const hasTextAtY = page.textElements.some(el => Math.abs(el.y - inputY) < 10 && el.text.trim().length > 0);
                            if (!hasTextAtY) {
                                bestMatch = {
                                    page: page.pageNumber,
                                    x: 71,
 ...
 */
function buildMatches(positions, fillData, targetPageNumber) {
    const matches = [];
 ...
 */
function observePlacements(filledContent, positions, fillData) {
    const observations = {};
    for (const [tag, pos] of Object.entries(positions)) {
        if (!pos || !fillData[tag]) {
