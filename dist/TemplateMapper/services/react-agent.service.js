...
exports.runReActAgent = runReActAgent;
const docx_utils_1 = require("../../shared/utils/docx.utils");
const llm_service_1 = require("./llm.service");
const tag_applicator_service_1 = require("./tag-applicator.service");
// ============================================================================
 ...
 */
async function runReActAgent(model, context) {
    const MAX_ITERATIONS_PER_SECTION = 3;
    const _SATISFACTION_THRESHOLD = 90; // Réservé pour usage futur
    const { tagContexts, extractedTags, templateCheckboxes, templateCheckboxPairs, targetParagraphs, targetCheckboxes, targetXml, docType: _docType, // Utilisé pour les logs de debug
    debug, } = context;
    // ========================================
    ...
    for (const currentSection of sectionsWithTags) {
        ...
        for (let sectionIteration = 0; sectionIteration < MAX_ITERATIONS_PER_SECTION; sectionIteration++) {
            ...
            const missingTagsInSection = sectionTags.filter((t) => t.status === 'pending' || t.status === 'failed');
            if (debug) {
            ...
            // -----------------------------------------
            const prompt = buildSectionPrompt(state, currentSection, missingTagsInSection, sectionParagraphsWithRelativeIndices, relativeTableMap, context.docType);
            if (debug) {
            ...
            let placements;
            try {
                llmResponse = await (0, llm_service_1.callConnectedLLM)(model, prompt);
                ...
                logAction(state, 'call_llm', { error: error.message }, 'failed');
                if (debug) {
            ...
            if (placements.length === 0) {
                ...
                placements = semanticFallbackMatchingBySection(missingTagsInSection, sectionParagraphs, targetParagraphs, relativeTableMap);
                if (debug) {
            ...
            // -----------------------------------------
            if (placements.length > 0) {
                const { xml: newXml, applied, failed } = (0, tag_applicator_service_1.applyTagsToTarget)(state.currentXml, placements, targetParagraphs);
    ...
    const verificationResult = verifyTagsInDocument(state.currentXml, state.expectedTags, state.foundTags, targetParagraphs);
    ...
    state.satisfaction = Math.round((state.tagsVerified / state.expectedTags.length) * 100);
    if (debug) {
        console.log(`\n\n📊 ═══════════════════════════════════════════`);
    ...
    const finalValidation = (0, docx_utils_1.validateDocxXml)(state.currentXml);
    if (debug) {
    ...
    const tagMatches = state.foundTags.map(f => ({
        tag: f.tag,
        targetParagraphIndex: findParagraphIndex(f, targetParagraphs),
        confidence: 0.9,
        insertionPoint: f.inTableCell ? 'table_cell' : 'after_colon',
    }));
    // Traiter les checkboxes (simplifié pour l'instant)
    const checkboxDecisions = processCheckboxes(templateCheckboxes, targetCheckboxes, templateCheckboxPairs);
    return {
 ...
 */
function buildExpectedTagsChecklist(tagContexts, extractedTags) {
    var _a;
    const checklist = [];
    const seenTags = new Set();
    for (const ctx of tagContexts) {
        if (seenTags.has(ctx.tag))
            continue;
    ...
    for (const tag of extractedTags) {
        ...
        checklist.push({
            ...
            expectedLocation: {
                type: 'text',
                nearText: (_a = tag.context) === null || _a === void 0 ? void 0 : _a.substring(0, 50),
            },
            templateContext: {
...
function extractTagsFromXml(xml) {
    ...
    while ((match = tagRegex.exec(xml)) !== null) {
        ...
        tags.push({
            ...
            context,
            inTableCell,
            tableIndex: tableInfo === null || tableInfo === void 0 ? void 0 : tableInfo.tableIndex,
            rowIndex: tableInfo === null || tableInfo === void 0 ? void 0 : tableInfo.rowIndex,
            columnIndex: tableInfo === null || tableInfo === void 0 ? void 0 : tableInfo.columnIndex,
        });
    }
    return tags;
}
 ...
 */
function isInTableCell(xml, position) {
 ...
 */
function getTableCellInfo(xml, position) {
 ...
 */
function verifyTagsInDocument(_xml, // Réservé pour analyses futures
expectedTags, foundTags, targetParagraphs) {
    const verifiedTags = [];
    const issues = [];
    for (const expected of expectedTags) {
        ...
        if (expected.expectedLocation.type === 'table_cell') {
            ...
            if (found.tableIndex !== expectedTable ||
            ...
            const hasContextMatch = checkSemanticMatch(found.context, expected.templateContext.labelBefore);
    ...
    for (const expected of tableCellTags) {
        if (expected.status === 'pending') {
            ...
            const targetCell = targetParagraphs.find(p => p.isTableCell &&
 ...
 */
function checkSemanticMatch(foundContext, expectedLabel) {
    if (!expectedLabel || expectedLabel.length < 5)
        return true;
 ...
 */
function buildAgentPrompt(state, missingTags, context) {
...
`;
    ...
    prompt += `\`\`\`json
${JSON.stringify(relevantParagraphs.map(p => ({
        idx: p.index,
...
function parseLLMResponse(response) {
    ...
    try {
        ...
        for (const item of placements) {
            ...
            if (confidence < 0.6)
                continue;
            let insertionPoint = 'after_colon';
            if (item.insertionPoint === 'table_cell')
                insertionPoint = 'table_cell';
            else if (item.insertionPoint === 'inline')
                insertionPoint = 'inline';
            else if (item.insertionPoint === 'replace_empty')
 ...
 */
function semanticFallbackMatching(missingTags, targetParagraphs) {
    const results = [];
    ...
    for (const tag of tableTags) {
        const loc = tag.expectedLocation;
        const matchingParagraph = targetParagraphs.find(p => p.isTableCell &&
            p.tableIndex === loc.tableIndex &&
            p.rowIndex === loc.rowIndex &&
            p.columnIndex === loc.columnIndex &&
    ...
    for (const tag of textTags) {
        ...
        for (const p of targetParagraphs) {
            if (usedParagraphs.has(p.index) || p.hasExistingTag)
                continue;
 ...
 */
function processCheckboxes(templateCheckboxes, targetCheckboxes, _pairs // Réservé pour le matching par paires
) {
    const decisions = [];
 ...
 */
function logAction(state, type, details, result) {
    state.actions.push({
        type,
 ...
 */
function findParagraphIndex(foundTag, targetParagraphs) {
    // Chercher par position de tableau
    if (foundTag.inTableCell && foundTag.tableIndex !== undefined) {
        const match = targetParagraphs.find(p => p.isTableCell &&
 ...
 */
function groupTagsBySection(expectedTags) {
    const groups = {};
 ...
 * la map sera { 7: 0, 8: 1 }
 */
function buildRelativeTableMap(sectionParagraphs) {
    const absoluteTableIndices = new Set();
 ...
 */
function applyRelativeTableIndices(paragraphs, relativeTableMap) {
 ...
 */
function convertRelativeToAbsoluteIndices(placements, sectionParagraphs, _allParagraphs) {
 ...
 */
function buildSectionPrompt(_state, // Réservé pour usage futur (ajout d'historique d'erreurs)
section, missingTags, sectionParagraphs, relativeTableMap, docType) {
    var _a, _b, _c;
...
`;
        for (const tag of tableTags) {
            const relTable = (_a = tag.relativeTableIndex) !== null && _a !== void 0 ? _a : '?';
            const row = (_b = tag.expectedLocation.rowIndex) !== null && _b !== void 0 ? _b : '?';
            const col = (_c = tag.expectedLocation.columnIndex) !== null && _c !== void 0 ? _c : '?';
...
\`\`\`json
${JSON.stringify(sectionParagraphs.slice(0, 50).map(p => ({
 ...
 */
function semanticFallbackMatchingBySection(missingTags, sectionParagraphs, allParagraphs, relativeTableMap) {
    const results = [];
    const usedParagraphs = new Set();
    // Inverser la map pour convertir les indices relatifs en absolus
    const absoluteTableMap = {};
    ...
    for (const tag of tableTags) {
        ...
        if (relTableIndex === undefined)
            continue;
        // Chercher dans les paragraphes de la section avec l'index relatif
        const matchingParagraph = sectionParagraphs.find(p => p.isTableCell &&
        ...
        if (matchingParagraph) {
            ...
            const globalParagraph = allParagraphs.find(ap => { var _a; return ap.xmlStart === ((_a = sectionParagraphs[matchingParagraph.index]) === null || _a === void 0 ? void 0 : _a.xmlStart); });
            if (globalParagraph) {
                results.push({
                    tag: tag.tag,
    ...
    for (const tag of textTags) {
        ...
        if (bestMatch) {
            ...
            const originalParagraph = allParagraphs.find(ap => { var _a; return ap.xmlStart === ((_a = sectionParagraphs[bestMatch.index]) === null || _a === void 0 ? void 0 : _a.xmlStart); });
