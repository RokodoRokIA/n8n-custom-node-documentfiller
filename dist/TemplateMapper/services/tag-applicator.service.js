...
exports.applyTagsToTarget = applyTagsToTarget;
 ...
 */
function applyTagsToTarget(xml, matches, targetParagraphs) {
    let modifiedXml = xml;
    const applied = [];
    const failed = [];
    // Suivi des tags et paragraphes déjà utilisés
    const usedTags = new Set();
    const modifiedParagraphs = new Map(); // index → liste des tags déjà insérés
    ...
    for (const pair of dateTagPairs) {
        ...
        if (!targetParagraph)
            continue;
        const paragraphXml = modifiedXml.substring(targetParagraph.xmlStart, targetParagraph.xmlEnd);
        const result = insertDatePairInExerciseLine(paragraphXml, pair.debutTag, pair.finTag);
        if (result.success) {
            modifiedXml =
    ...
    const sortedMatches = [...matches].sort((a, b) => b.targetParagraphIndex - a.targetParagraphIndex);
    for (const match of sortedMatches) {
        ...
        const result = applyTagToParagraph(paragraphXml, fullTag, match.insertionPoint, targetParagraph);
        if (result.success) {
            ...
            applied.push(`${match.tag} → paragraphe ${match.targetParagraphIndex} (${match.insertionPoint})`);
 ...
 */
function applyTagToParagraph(paragraphXml, tag, insertionPoint, paragraph) {
 ...
 */
function insertTagAfterColon(paragraphXml, tag) {
    // Trouver le dernier <w:t> qui contient ":"
    const textTagRegex = /<w:t([^>]*)>([^<]*)<\/w:t>/g;
    let lastColonMatch = null;
    let match;
    ...
    const newXml = paragraphXml.replace(lastColonMatch.full, `<w:t${lastColonMatch.attrs}>${newText}</w:t>`);
    return {
 ...
 */
function insertTagInEmptyCell(paragraphXml, tag, text) {
    ...
    if (runMatch) {
        const newXml = paragraphXml.replace(runMatch[0], `${runMatch[1]}<w:t>${tag}</w:t>${runMatch[2]}`);
        return { success: true, newXml };
    }
    return { success: false, newXml: paragraphXml };
}
 ...
 */
function insertTagInline(paragraphXml, tag) {
    ...
    const newXml = paragraphXml.replace(lastMatch.full, `<w:t${lastMatch.attrs}>${newText}</w:t>`);
    return { success: true, newXml };
}
/**
 * Insère un tag pour une case à cocher (checkbox).
 */
function insertTagForCheckbox(paragraphXml, tag) {
 ...
 */
function insertTagInTableCell(paragraphXml, tag) {
    // Extraire le texte actuel pour analyser la cellule
    const textContent = (0, utils_1.extractTextFromXml)(paragraphXml).trim();
    ...
    if (textContent === '%' || textContent.endsWith('%')) {
        ...
        if (percentElement) {
            ...
            const newXml = paragraphXml.replace(percentElement.full, `<w:t${percentElement.attrs}>${newText}</w:t>`);
            return { success: true, newXml };
        }
    }
    ...
    if (textContent.endsWith(':')) {
        ...
        const newXml = paragraphXml.replace(lastText.full, `<w:t${lastText.attrs}>${newText}</w:t>`);
        return { success: true, newXml };
    }
    // CAS 4: Cellule presque vide (< 10 caractères sans mots significatifs)
    if (textContent.length < 10 && !/[a-zA-ZÀ-ÿ]{3,}/.test(textContent)) {
        const lastText = textMatches[textMatches.length - 1];
        const newXml = paragraphXml.replace(lastText.full, `<w:t${lastText.attrs}>${tag}</w:t>`);
        return { success: true, newXml };
    }
 ...
 */
function insertTagInEmptyTableCell(paragraphXml, tag, textMatches) {
    var _a, _b;
    // S'il y a déjà un <w:t>, l'utiliser
    if (textMatches.length > 0) {
        const firstText = textMatches[0];
        const newXml = paragraphXml.replace(firstText.full, `<w:t${firstText.attrs}>${tag}</w:t>`);
        return { success: true, newXml };
    }
    ...
    if (runMatch) {
        ...
        if (/<w:rPr>/.test(runContent)) {
            ...
            if (rPrEndIndex !== -1) {
                const newRunContent = runContent.substring(0, rPrEndIndex + 8) + `<w:t>${tag}</w:t>`;
                const runAttrs = ((_a = runMatch[0].match(/<w:r([^>]*)>/)) === null || _a === void 0 ? void 0 : _a[1]) || '';
                const newXml = paragraphXml.replace(runMatch[0], `<w:r${runAttrs}>${newRunContent}</w:r>`);
                return { success: true, newXml };
            }
        }
        // Pas de <w:rPr>, ajouter <w:t> directement
        const runAttrs = ((_b = runMatch[0].match(/<w:r([^>]*)>/)) === null || _b === void 0 ? void 0 : _b[1]) || '';
        const newXml = paragraphXml.replace(runMatch[0], `<w:r${runAttrs}>${runContent}<w:t>${tag}</w:t></w:r>`);
        return { success: true, newXml };
    }
    ...
    const beforeClose = paragraphXml.substring(0, paragraphXml.lastIndexOf(closingTag));
 ...
 */
function updateParagraphPositions(paragraphs, modifiedIndex, delta) {
 ...
 */
function groupDateTagPairs(matches) {
    const pairs = [];
    ...
    for (const debutMatch of debutMatches) {
        ...
        const finMatch = matches.find((m) => m.tag === finTag && m.targetParagraphIndex === debutMatch.targetParagraphIndex);
        if (finMatch) {
            pairs.push({
 ...
 * @returns Le résultat de l'insertion
 */
function insertDatePairInExerciseLine(paragraphXml, debutTag, finTag) {
    ...
    const textContent = (0, utils_1.extractTextFromXml)(paragraphXml);
    if (!exercisePattern.test(textContent)) {
        ...
        if (simplePattern.test(textContent)) {
            ...
            newXml = newXml.replace(/(<w:t[^>]*>)([^<]*du\s*)([^<]*au\s*)([^<]*)(<\/w:t>)/gi, `$1$2${fullDebutTag} $3${fullFinTag}$4$5`);
    ...
    newXml = newXml.replace(/(<w:t[^>]*>)([^<]*)(du\s*)([\.…]+|\s{5,})(\s*au\s*)([\.…]+|\s{5,})([^<]*)(<\/w:t>)/gi, `$1$2$3${fullDebutTag}$5${fullFinTag}$7$8`);
    if (newXml !== paragraphXml) {
        return { success: true, newXml };
    }
