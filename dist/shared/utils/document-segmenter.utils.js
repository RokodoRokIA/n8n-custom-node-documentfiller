...
exports.extractRelevantSegments = extractRelevantSegments;
 ...
 */
function segmentDocument(xml, options = {}) {
    const { strategy = 'auto', includeEmpty = false, minSegmentSize = 50, mergeSmallSegments = true, } = options;
...
function segmentByTables(xml) {
    ...
    for (const match of tableMatches) {
        ...
        const text = (0, xml_utils_1.extractTextFromXml)(tableXml);
        // Extraire les tags
        ...
        segments.push({
            ...
            type: 'table',
            title,
            sectionLetter: sectionMatch === null || sectionMatch === void 0 ? void 0 : sectionMatch.letter,
            tableIndex,
            xml: tableXml,
            text,
            startOffset,
 ...
 */
function createTableTitle(text, index, sectionMatch) {
    if (sectionMatch) {
 ...
 */
function segmentBySections(xml) {
    const segments = [];
    const text = (0, xml_utils_1.extractTextFromXml)(xml);
    ...
    for (let i = 0; i < sectionPositions.length; i++) {
        ...
        const segmentText = (0, xml_utils_1.extractTextFromXml)(segmentXml);
        segments.push({
 ...
 */
function enrichSegmentMetadata(segment) {
    const text = segment.text.toLowerCase();
    segment.metadata = {
        ...segment.metadata,
        // Catégories de contenu
        hasFinancialData: /chiffre\s*d'affaires|CA\s|exercice\s*du|montant/i.test(text),
        hasContactInfo: /email|@|téléphone|télécopie|fax|adresse\s*(électronique|postale)/i.test(text),
 ...
 */
function calculateRelevanceScore(segment) {
    let score = 0;
    // Tags présents = très pertinent
    score += segment.tags.length * 20;
    // Contenu identifiable
    if (segment.metadata.hasIdentification)
        score += 15;
    if (segment.metadata.hasContactInfo)
        score += 10;
    if (segment.metadata.hasFinancialData)
        score += 15;
    if (segment.metadata.hasLegalInfo)
        score += 5;
    // Taille du contenu
    if (segment.text.length > 500)
        score += 5;
    if (segment.paragraphCount > 5)
        score += 5;
 ...
 */
function extractTagsFromSegment(xml) {
    const tags = [];
    let match;
 ...
 */
function createFullDocumentSegment(xml) {
    const text = (0, xml_utils_1.extractTextFromXml)(xml);
    return {
        id: 'full-document',
 ...
 */
function mergeSmallConsecutiveSegments(segments, minSize) {
 ...
 */
function matchSegments(templateSegments, targetSegments) {
 ...
 */
function calculateSegmentSimilarity(template, target) {
    let score = 0;
    ...
    const templateWords = new Set(template.text
        .toLowerCase()
        .split(/\s+/)
        .filter((w) => w.length > 4));
    const targetWords = new Set(target.text
        .toLowerCase()
        .split(/\s+/)
        .filter((w) => w.length > 4));
    let commonWords = 0;
 ...
 */
function extractRelevantSegments(result, options = {}) {
    ...
    return result.segments.filter((segment) => {
        ...
        const relevance = segment.metadata.relevanceScore || 0;
