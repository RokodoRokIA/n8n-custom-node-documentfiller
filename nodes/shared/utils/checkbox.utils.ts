 ...
 */
export function extractCheckboxes(xml: string): ExtractedCheckbox[] {
 ...
 */
function findUnicodeCheckboxes(
	text: string,
	paragraphIndex: number,
	...
	const results: ExtractedCheckbox[] = [];
 ...
 */
export function findCheckboxPairs(checkboxes: ExtractedCheckbox[]): CheckboxPair[] {
	const pairs: CheckboxPair[] = [];
	...
	for (let i = 0; i < checkboxes.length; i++) {
		...
		for (let j = i + 1; j < checkboxes.length; j++) {
			...
			if (isOuiNonPair && isClose) {
				...
				if (oui.checked && !non.checked) value = true;
				else if (!oui.checked && non.checked) value = false;
 ...
 */
function isOuiCheckbox(label: string): boolean {
 ...
 */
function isNonCheckbox(label: string): boolean {
	const normalized = label.toLowerCase().trim();
	return (
		normalized === 'non' ||
...
function extractQuestionContext(
	checkbox: ExtractedCheckbox,
	allCheckboxes: ExtractedCheckbox[],
	currentIndex: number
): string {
 ...
 */
export function generateCheckboxTags(
	checkboxes: ExtractedCheckbox[],
	pairs: CheckboxPair[]
): Map<string, { checked: boolean; label: string; type: string }> {
	const tags = new Map<string, { checked: boolean; label: string; type: string }>();
 ...
 */
export function booleanToCheckbox(checked: boolean, style: 'unicode' | 'text' | 'boolean' = 'unicode'): string {
	switch (style) {
		case 'unicode':
			return checked ? '☑' : '☐';
		case 'text':
...
// ============================================================================
// APPLICATION DES CHECKBOXES AU DOCUMENT CIBLE
// ============================================================================
 ...
 */
export function matchCheckboxes(
 ...
 */
export function applyCheckboxesToXml(
	targetXml: string,
	matches: CheckboxMatch[]
 ...
 * </w:ffData>
 */
function setFormCheckboxState(paragraphXml: string, checked: boolean): string {
	...
	if (modified.includes('w14:checked')) {
		modified = modified.replace(
			/w14:checked\s+w14:val="[01]"/g,
			`w14:checked w14:val="${newVal}"`
		);
	}
 ...
 */
function setUnicodeCheckboxState(paragraphXml: string, checked: boolean): string {
 ...
 */
function normalizeForMatching(label: string): string {
	return label
		.toLowerCase()
		.replace(/[☑☐✓✔□■○◯◻︎]/g, '')
 ...
 */
function isOuiText(text: string): boolean {
 ...
 */
function isNonText(text: string): boolean {
	const normalized = text.trim().toLowerCase();
 ...
 */
function calculateMatchScore(label1: string, label2: string): number {
