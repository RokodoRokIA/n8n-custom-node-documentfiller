...
import { TagContext, TargetParagraph, MatchResult, InsertionPoint } from '../../shared/types';
 ...
 */
const DC2_LABELS: Record<string, string[]> = {
 ...
 */
export function patternBasedMatching(
...
function processExerciseDates(
	...
	const exerciseLines = targetParagraphs.filter(p =>
		p.text.toLowerCase().includes('exercice') &&
		p.text.includes('du') &&
		p.text.includes('au') &&
		p.isTableCell
...
function processCAValues(
	...
	const caLabelIndex = targetParagraphs.findIndex(p =>
		normalizeApostrophes(p.text.toLowerCase()).includes('chiffre d\'affaires global') &&
		p.isTableCell
	);

	if (caLabelIndex === -1) return;

	// Trouver le label "Part du chiffre d'affaires" (limite de recherche)
	const partCaLabelIndex = targetParagraphs.findIndex(p =>
		normalizeApostrophes(p.text.toLowerCase()).includes('part du chiffre d\'affaires') &&
		p.isTableCell &&
	...
	const existingCATags = caTags.filter(tag =>
		tagContexts.some(tc => tc.tag === tag)
	);

	// Trouver les cellules vides ENTRE les deux labels
	const maxIndex = partCaLabelIndex !== -1 ? partCaLabelIndex : caLabelIndex + 10;

	const emptyCellsInRow = targetParagraphs.filter(p =>
		p.index > caLabelIndex &&
		p.index < maxIndex &&
		p.isTableCell &&
...
function processPartCAValues(
	...
	if (partCaLabelIndex === -1) return;

	// Les tags PART_CA à placer
	const partCaTags = ['PART_CA_N', 'PART_CA_N1', 'PART_CA_N2'];
	const existingPartCATags = partCaTags.filter(tag =>
		tagContexts.some(tc => tc.tag === tag)
	);

	// Trouver les cellules avec "%" APRÈS le label
	const percentCells = targetParagraphs.filter(p =>
		p.index > partCaLabelIndex &&
		p.isTableCell &&
		p.text.trim() === '%' &&
		!usedParagraphs.has(p.index)
	).sort((a, b) => a.index - b.index);
	...
	for (let i = 0; i < Math.min(existingPartCATags.length, percentCells.length); i++) {
		...
		if (cell.index - partCaLabelIndex > 15) continue;
 ...
 */
function processIdentificationTags(
	tagContexts: TagContext[],
	targetParagraphs: TargetParagraph[],
	matches: MatchResult[],
	usedParagraphs: Set<number>,
	...
	for (const tagName of identificationTags) {
		...
		if (tagName === 'NOM_COMMERCIAL') {
			const nomCommParagraphs = targetParagraphs.filter(p =>
				normalizeApostrophes(p.text.toLowerCase()).includes('nom commercial') &&
			...
			for (const label of labels) {
				...
				let found = targetParagraphs.findIndex(p =>
					normalizeApostrophes(p.text.toLowerCase()).startsWith(label.toLowerCase()) &&
					p.text.trim().endsWith(':') &&
					!usedParagraphs.has(p.index)
				);
				...
				if (found === -1) {
					found = targetParagraphs.findIndex(p =>
						normalizeApostrophes(p.text.toLowerCase()).includes(label.toLowerCase()) &&
						p.text.trim().endsWith(':') &&
						p.text.length < 150 &&
						!usedParagraphs.has(p.index)
					);
		...
		if (labelParagraphIndex === -1) continue;
		...
		const nextEmptyCell = targetParagraphs.find(p =>
			p.index > labelParagraphIndex &&
		...
		const specificLabelPara = targetParagraphs.find(p =>
			p.index > labelParagraphIndex &&
...
function processRemainingTags(
	...
	for (const tagContext of tagContexts) {
		...
		if (tag.match(/^CA_N\d?_(DEBUT|FIN)$/)) continue;
		...
		for (const para of targetParagraphs) {
			...
			if (para.hasExistingTag) continue;
			if (para.text.length < 3) continue;
