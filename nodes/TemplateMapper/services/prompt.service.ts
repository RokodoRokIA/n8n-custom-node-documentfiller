 ...
 */
export function generateTransferLearningPrompt(
 ...
 */
function buildFewShotExamples(contexts: TagContext[], targetList: TargetListItem[]): string {
...
function findBestCandidate(
	...
	let bestMatch: TargetListItem | null = null;
	let bestScore = 0;
...
/**
 * Prépare les paragraphes cibles avec métadonnées.
 */
...
function prepareTargetParagraphs(paragraphs: TargetParagraph[]): TargetListItem[] {
	return paragraphs
 ...
 */
export function generateCheckboxFewShot(
 ...
 */
export function generateDirectMappingPrompt(
	targetParagraphs: TargetParagraph[],
	extractedTags: ExtractedTag[],
	docType: DocumentType
): string {
 ...
 */
export function validatePromptSize(prompt: string, strict = false): string {
	const size = prompt.length;

	if (size > MAX_PROMPT_SIZE) {
		console.warn(
			`⚠️ ATTENTION: Le prompt fait ${Math.round(size / 1000)}KB, ` +
