...
import {
	...
	SegmentationResult,
	segmentDocument,
	matchSegments as matchDocumentSegments,
} from '../../shared/utils/document-segmenter.utils';
import {
	TagContext,
	ExtractedTag,
	TargetParagraph,
...
import { extractTagContextsFromTemplate, extractTargetParagraphs } from '../../shared';
 ...
 */
let globalTargetParagraphsCache: TargetParagraph[] | null = null;
let cachedTargetXml: string | null = null;
 ...
 * }
 */
export function prepareSegmentMatchingPlan(
	templateXml: string,
	targetXml: string,
	...
	const templateResult = segmentDocument(templateXml, { strategy: 'hybrid' });
	const targetResult = segmentDocument(targetXml, { strategy: 'hybrid' });
	...
	const segmentMatches = matchDocumentSegments(templateResult.segments, targetResult.segments);

	// Étape 4: Préparer les paires de segments
	const matchedPairs: MatchedSegmentPair[] = [];
 ...
 */
function findBestCandidateInSegment(
	ctx: TagContext,
	paragraphs: { idx: number; text: string; isCell: boolean; endsWithColon: boolean }[]
	...
	let best: { idx: number; text: string } | null = null;
	let bestScore = 0;
	...
	for (const p of paragraphs) {
		...
		for (const w of words) {
			if (textLower.includes(w)) score += w.length;
		}
		if (p.endsWithColon) score += 5;
 ...
 */
function extractTagContextsFromSegment(segment: DocumentSegment): TagContext[] {
	// Utiliser la fonction existante sur le XML du segment
	return extractTagContextsFromTemplate(segment.xml);
}
 ...
 */
function extractParagraphsFromSegment(
	segment: DocumentSegment,
	globalTargetXml: string
): TargetParagraph[] {
	...
	if (needsRefresh) {
		globalTargetParagraphsCache = extractTargetParagraphs(globalTargetXml);
		cachedTargetXml = globalTargetXml;
	...
	const paragraphs = globalTargetParagraphsCache ?? [];
 ...
 */
export function resetParagraphCache(): void {
 ...
 */
export function combineSegmentResults(
	segmentResults: Map<string, TagMatch[]>,
	matchedPairs: MatchedSegmentPair[]
): TagMatch[] {
 ...
 */
export function logMatchingPlan(plan: SegmentMatchingPlan): void {
 ...
 */
export function shouldUseSegmentation(
	templateXml: string,
	extractedTags: ExtractedTag[]
): boolean {
