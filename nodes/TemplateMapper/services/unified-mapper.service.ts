 ...
 */
export async function performUnifiedMapping(
	model: LLMModel,
	context: MappingContext
): Promise<UnifiedMappingResult> {
	const {
		tagContexts,
		extractedTags,
		templateCheckboxes,
	...
	let iteration = 0;
	let lastParsed: { tagMatches: MatchResult[]; checkboxDecisions: CheckboxDecision[] } | null = null;
	...
	while (iteration < MAX_ITERATIONS) {
		...
		const prompt = buildUnifiedPrompt(
			...
			templateCheckboxPairs,
			docType,
			correctionFeedback // Ajouter le feedback de correction si disponible
		);
		...
		try {
			...
			// ============================================================
			const issues = validateMappingResults(
				parsed.tagMatches,
				tagContexts,
				targetParagraphs,
				parsed.checkboxDecisions,
				templateCheckboxPairs
			);
			...
			console.error(`   ❌ Erreur itération ${iteration}:`, (error as Error).message);
			break;
		}
	}
...
function validateMappingResults(
	...
	const paragraphCounts = new Map<number, string[]>();
		...
		if (tags.length > 1) {
			...
			if (!paragraph?.isTableCell) {
				...
				const sameCell = contexts.every(c =>
					c?.tableIndex === contexts[0]?.tableIndex &&
					c?.rowIndex === contexts[0]?.rowIndex &&
					c?.columnIndex === contexts[0]?.columnIndex
				);
	...
	const cellGroups = new Map<string, TagContext[]>();
	...
	for (const [cellKey, tagsInCell] of cellGroups) {
		if (tagsInCell.length > 1) {
			...
			for (const ctx of tagsInCell) {
				...
				if (match) matchedTo.add(match.targetParagraphIndex);
			}
	...
	for (const match of tagMatches) {
		...
		if (ctx.tableIndex !== undefined) continue;
		...
		if (labelKeywords.length >= 2 && matchScore < 4) {
			issues.push(
				`Tag {{${match.tag}}} (label: "${ctx.labelBefore.substring(0, 30)}...") ` +
	...
	if (checkboxDecisions.length > 0) {
		...
		for (let i = 0; i < checkboxDecisions.length - 1; i++) {
			...
			const isOuiNonPair =
				(currentLabel === 'oui' && nextLabel === 'non') ||
			...
			if (isOuiNonPair) {
				...
				if (current.shouldBeChecked && next.shouldBeChecked) {
					issues.push(
						`Checkboxes Oui/Non (idx ${current.targetIndex} + ${next.targetIndex}): ` +
		...
		for (const pair of templateCheckboxPairs) {
			...
			const ouiDecision = checkboxDecisions.find(d =>
				d.label.toLowerCase().includes('oui')
			);
			const nonDecision = checkboxDecisions.find(d =>
				d.label.toLowerCase().includes('non')
			);
			...
			if (ouiDecision && nonDecision) {
				...
				if (pair.value === true && !ouiDecision.shouldBeChecked) {
					issues.push(
						`Checkbox "Oui" devrait être cochée (template: ${pair.question.substring(0, 30)}...)`
					);
				}
				if (pair.value === false && !nonDecision.shouldBeChecked) {
					issues.push(
						`Checkbox "Non" devrait être cochée (template: ${pair.question.substring(0, 30)}...)`
					);
				}
			}
		}
 ...
 */
function extractSemanticKeywords(label: string): string[] {
...
Tu dois trouver le MÊME CONTEXTE dans le document cible.`;
	...
	const checkboxSection = buildCheckboxFewShotSection(
		templateCheckboxes,
		targetCheckboxes,
		templateCheckboxPairs
	);
 ...
 */
function buildTagFewShotSection(
	tagContexts: TagContext[],
	targetParagraphs: TargetParagraph[],
	extractedTags: ExtractedTag[]
): string {
 ...
 */
function buildCheckboxFewShotSection(
	templateCheckboxes: ExtractedCheckbox[],
	targetCheckboxes: ExtractedCheckbox[],
	pairs: CheckboxPair[]
): string {
	...
	for (const cb of templateCheckboxes.slice(0, 15)) {
		...
		examples.push(
			`- ${status}: "${cb.label.substring(0, 50)}" → Candidat: ${candidateInfo}`
		);
	}
 ...
 */
function parseUnifiedResponse(response: string): {
	tagMatches: MatchResult[];
	checkboxDecisions: CheckboxDecision[];
} {
	const result = {
		tagMatches: [] as MatchResult[],
		checkboxDecisions: [] as CheckboxDecision[],
	};
 ...
 */
function validateTagMatch(item: Record<string, unknown>): MatchResult | null {
	if (!item.tag || typeof item.tag !== 'string') return null;

	const idx = (item.targetIdx ?? item.targetParagraphIndex) as number | undefined;
	if (idx === undefined || typeof idx !== 'number' || idx < 0) return null;

	const confidence = (item.confidence as number) || 0.8;
	if (confidence < 0.7) return null;

	const insertionPoint = normalizeInsertionPoint(item.insertionPoint as string);
 ...
 */
function validateCheckboxDecision(item: Record<string, unknown>): CheckboxDecision | null {
	const idx = (item.targetIdx ?? item.targetIndex ?? item.idx) as number | undefined;
	if (idx === undefined || typeof idx !== 'number' || idx < 0) return null;

	const checked = item.checked ?? item.shouldBeChecked;
...
function normalizeInsertionPoint(point: string | undefined): InsertionPoint {
	const valid: InsertionPoint[] = ['after_colon', 'table_cell', 'replace_empty', 'inline', 'checkbox'];
	if (point && valid.includes(point as InsertionPoint)) {
...
function findBestCandidate(
	ctx: TagContext,
	targetParagraphs: TargetParagraph[],
	usedParagraphs: Set<number> = new Set()
	...
	for (const p of targetParagraphs) {
		if (p.hasExistingTag) continue;
		if (usedParagraphs.has(p.index)) continue;
		...
		if (ctx.section && p.section === ctx.section) score += 3;
		if (ctx.type === 'table_cell' && p.isTableCell) score += 5;
 ...
 */
function findMatchingCheckbox(
	templateCb: ExtractedCheckbox,
	targetCheckboxes: ExtractedCheckbox[]
): ExtractedCheckbox | null {
	...
	let bestMatch: ExtractedCheckbox | null = null;
	let bestScore = 0;
	...
	for (const cb of targetCheckboxes) {
		...
		for (const word of words) {
			if (targetLabel.includes(word)) score += word.length;
		}
...
function isOuiLabel(label: string): boolean {
	return label === 'oui' || label.startsWith('oui ') || /^\s*oui\s*$/.test(label);
}

function isNonLabel(label: string): boolean {
 ...
 */
function performSemanticFallback(context: MappingContext): UnifiedMappingResult {
	...
	const checkboxDecisions = semanticCheckboxMatching(
		context.templateCheckboxes,
		context.targetCheckboxes
	);
...
function semanticTagMatching(
	...
	for (const [cellKey, tagsInCell] of cellGroups) {
		...
		if (!firstTag || usedTags.has(firstTag.tag)) continue;

		const positionMatch = targetParagraphs.find(
			(p) =>
				p.isTableCell &&
		...
		if (positionMatch) {
			...
			for (const ctx of tagsInCell) {
				if (usedTags.has(ctx.tag)) continue;
	...
	for (const ctx of tagContexts) {
		...
		if (candidate) {
			let insertionPoint: InsertionPoint = 'inline';
			if (candidate.text.trim().endsWith(':')) insertionPoint = 'after_colon';
			else if (candidate.isTableCell) insertionPoint = 'table_cell';
			else if (candidate.text.trim().length < 5) insertionPoint = 'replace_empty';
 ...
 */
function semanticCheckboxMatching(
	templateCheckboxes: ExtractedCheckbox[],
	targetCheckboxes: ExtractedCheckbox[]
): CheckboxDecision[] {
 ...
 */
export function applyCheckboxDecisions(
	xml: string,
	...
	return applyCheckboxesToXml(xml, matches);
}
