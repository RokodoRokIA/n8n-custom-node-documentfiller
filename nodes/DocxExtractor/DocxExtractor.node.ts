...
import { extractDocxContent } from './services';
import type { ExtractionOptions } from '../shared/types/extraction.types';
...
export class DocxExtractor implements INodeType {
	...
	description: INodeTypeDescription = {
		...
		// Description
		description:
			'Extrait le contenu structuré d\'un document Word (DOCX) vers JSON. ' +
		...
		properties: [
			// ==================== DOCUMENT SOURCE ====================
			{
				...
				required: true,
				description:
					'Nom de la propriété binaire contenant le fichier DOCX à extraire.',
			},
				...
				options: [
					{
						...
						value: 'hierarchical',
						description:
							'Organise le contenu en sections et sous-sections. ' +
					...
					},
					{
						...
						value: 'flat',
						description:
							'Liste simple de tous les éléments (paragraphes, titres, listes). ' +
				...
				options: [
					{
						...
						default: true,
						description:
							'Inclure les métadonnées du document (titre, auteur, dates, compteurs).',
					},
					{
						...
						default: false,
						description:
							'Inclure les informations de style (gras, italique, souligné) pour chaque élément.',
					},
					{
						...
						options: [
							{
								...
								value: 'array',
								description:
									'Retourne les tableaux comme des arrays de lignes avec des cellules.',
							},
							{
								name: 'Array d\'Objets (si headers)',
								value: 'objects',
								description:
									'Si la première ligne est détectée comme header, ' +
 ...
 */
async function processDocxExtractorItem(
	...
	const outputFormat = this.getNodeParameter('outputFormat', itemIndex) as 'hierarchical' | 'flat';
	...
	if (!binaryData || !binaryData[binaryProperty]) {
		throw new NodeOperationError(
			...
			`Aucun document trouvé dans la propriété binaire "${binaryProperty}". ` +
				'Vérifiez que le nœud précédent fournit bien un fichier DOCX.',
			{ itemIndex }
		);
	}
	...
	if (
		...
		throw new NodeOperationError(
			this.getNode(),
			`Le fichier "${originalFilename}" n'est pas un document Word (.docx). ` +
				'Ce nœud ne supporte que les fichiers DOCX.',
			{ itemIndex }
		);
	}
	...
	const result = await extractDocxContent(docxBuffer, extractionOptions);

	// 7. Vérifier le résultat
	if (!result.success) {
		throw new NodeOperationError(
			this.getNode(),
			`L'extraction du document a échoué: ${result.error}`,
			{ itemIndex }
		);
	}
