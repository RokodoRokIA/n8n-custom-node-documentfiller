...
import { callConnectedLLM } from '../../TemplateMapper/services/llm.service';
 ...
 */
export async function runPdfMatchingAgent(
	model: LLMModel,
	...
	const matches: PdfMatchResult[] = [];

	// Phase 1: Matching par position directe
	if (debug) console.log('\n   Phase 1: Matching par position...');
	const positionMatches = matchByPosition(
		state.expectedPlacements,
		targetContent,
		positionTolerance
	);
	...
	if (stillUnmatched.length > 0) {
		...
		const llmMatches = await matchWithLLM(
			model,
			stillUnmatched,
			targetContent,
			templateContent,
 ...
 */
function buildExpectedPlacements(templateContent: PdfDocumentContent): PdfExpectedPlacement[] {
	const placements: PdfExpectedPlacement[] = [];

	if (!templateContent.tags) return placements;
...
function updatePlacementStatus(
	state: PdfAgentState,
	matches: PdfMatchResult[],
	newStatus: PdfExpectedPlacement['status']
): void {
...
function matchByPosition(
	...
	for (const placement of expectedPlacements) {
		...
		let bestMatch: PdfField | null = null;
		let bestScore = 0;
		...
		for (const field of targetPage.fields) {
			...
			// Distance relative
			const relDistance = Math.sqrt(
				Math.pow(templateRelX - targetRelX, 2) +
...
function matchBySemantic(
	...
	const allTargetFields: PdfField[] = [];
	...
	for (const placement of expectedPlacements) {
		...
		if (templateKeywords.length === 0) continue;
...
async function matchWithLLM(
	model: LLMModel,
	unmatchedPlacements: PdfExpectedPlacement[],
	targetContent: PdfDocumentContent,
	templateContent: PdfDocumentContent,
	debug: boolean
): Promise<PdfMatchResult[]> {
	if (unmatchedPlacements.length === 0) return [];
 ...
 */
function buildLLMMatchingPrompt(
	unmatchedPlacements: PdfExpectedPlacement[],
	targetContent: PdfDocumentContent,
	_templateContent: PdfDocumentContent
): string {
	// Collecter les champs cibles disponibles
	const targetFields: { id: string; label: string; page: number; x: number; y: number }[] = [];
	for (const page of targetContent.pages) {
 ...
 */
function parseLLMMatchingResponse(
	response: string,
	_unmatchedPlacements: PdfExpectedPlacement[],
	targetContent: PdfDocumentContent
): PdfMatchResult[] {
	const results: PdfMatchResult[] = [];
	...
	try {
		...
		const fieldIndex = new Map<string, PdfField>();
		for (const page of targetContent.pages) {
			for (const field of page.fields) {
		...
		for (const match of matches) {
			if (!match.tag || !match.fieldId) continue;
		...
		console.error('Erreur parsing réponse LLM:', (error as Error).message);
	}
 ...
 */
export function validateMatches(matches: PdfMatchResult[]): {
	valid: PdfMatchResult[];
	conflicts: { tag1: string; tag2: string; fieldId: string }[];
} {
	const valid: PdfMatchResult[] = [];
	const usedFields = new Map<string, string>(); // fieldId -> tag
	const conflicts: { tag1: string; tag2: string; fieldId: string }[] = [];
