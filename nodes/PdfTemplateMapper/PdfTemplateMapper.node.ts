...
import { extractPdfContent, runPdfReActAgent, type PdfMappingContext } from './services';
...
const DEFAULT_FIELD_CONFIG: Record<string, { labels: string[]; type: 'inline' | 'multiline' | 'checkbox'; minGap?: number }> = {
...
export class PdfTemplateMapper implements INodeType {
...
async function processItem(
	...
	// Paramètres
	const pdfProperty = ctx.getNodeParameter('pdfProperty', itemIndex) as string;
	const pageNumber = ctx.getNodeParameter('pageNumber', itemIndex) as number;
	const fillDataRaw = ctx.getNodeParameter('fillData', itemIndex) as string;
	const fieldConfigRaw = ctx.getNodeParameter('fieldConfig', itemIndex, '') as string;
	const sectionMarker = ctx.getNodeParameter('sectionMarker', itemIndex, '') as string;
	...
	if (!binary?.[pdfProperty]) {
		throw new NodeOperationError(
			ctx.getNode(),
			`PDF non trouvé dans le champ "${pdfProperty}"`,
			{ itemIndex }
		);
	}
	...
	let fillData: Record<string, string | boolean> = {};
	try {
		fillData = JSON.parse(fillDataRaw);
	} catch {
		throw new NodeOperationError(
			ctx.getNode(),
			'Le champ "Données à remplir" doit contenir un JSON valide',
			{ itemIndex }
		);
	}
	...
	let customFieldConfig: Record<string, { labels: string[]; type: 'inline' | 'multiline' | 'checkbox'; minGap?: number }> = {};
	if (fieldConfigRaw && typeof fieldConfigRaw === 'object') {
		customFieldConfig = fieldConfigRaw as typeof customFieldConfig;
		...
		try {
			...
			throw new NodeOperationError(
				ctx.getNode(),
				'Le champ "Configuration des champs" doit contenir un JSON valide. Exemple: {"MON_CHAMP": {"labels": ["Label"], "type": "checkbox"}}',
				{ itemIndex }
			);
		}
	}
	...
	if (fieldsWithValues.length === 0) {
		throw new NodeOperationError(
			ctx.getNode(),
			'Aucune donnée à remplir: tous les champs du JSON sont vides. ' +
			'Veuillez fournir au moins une valeur non-vide (string) ou true (checkbox). ' +
			`Champs détectés: ${Object.keys(fillData).join(', ') || 'aucun'}`,
			{ itemIndex }
		);
	}
	...
	if (!model) {
		throw new NodeOperationError(
			ctx.getNode(),
			'Aucun modèle LLM connecté. Connectez un noeud LLM au port "Model" (ex: OpenAI, Claude, Gemini, etc.).',
			{ itemIndex }
		);
	}
	...
	if (!targetPage) {
		throw new NodeOperationError(
			ctx.getNode(),
			`Page ${pageNumber} non trouvée. Le document a ${content.pageCount} pages.`,
			{ itemIndex }
		);
	}
	...
	const mappingContext: PdfMappingContext = {
		pdfContent: content,
	...
	const agentResult = await runPdfReActAgent(model, mappingContext, pdfBuffer);
